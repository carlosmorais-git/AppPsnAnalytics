...
interface BarChartDataItemProps {
  genre?: string;
  platform?: string;
  range?: string;
  title?: string;
...
interface SimpleBarChartProps {
  data: BarChartDataItemProps[];
  title: string;
  color?: string;
}
interface LineChartDataItemProps {
  month: string;
  avgPrice: number;
}
...
const AnalyticsScreen = () => {
  ...
  const { analytics, loading, refreshing, refreshGames } = useGames();

  // Componente para gráfico de barras simples (memoizado)
  const SimpleBarChart = useCallback(
    ({ data, title, color = "#60a5fa" }: SimpleBarChartProps) => {
      return (
  ...
  // Componente para gráfico de linha simples (memoizado)
  const SimpleLineChart = useCallback(
    ({ data, title }: { data: LineChartDataItemProps[]; title: string }) => {
  ...
  // Componente para card de estatística (memoizado)
  const StatCard = useCallback(
    ({ title, value, subtitle, color = "#60a5fa" }: StatCardProps) => (
    ...
    >
      ...
      {analytics && (
        <>
          ...
          {analytics.generalStats && (
            <View style={styles.section}>
              ...
              <View style={styles.statsGrid}>
                <StatCard
                  title="Melhor Oferta"
                  value={analytics.generalStats?.bestDeal || "N/A"}
                  subtitle={`${
                    analytics.generalStats?.bestDealDiscount || 0
                  }% de desconto`}
                  color="#10b981"
                />
                <StatCard
          ...
          {analytics.priceEvolution && analytics.priceEvolution.length > 0 && (
            <View style={styles.section}>
              <SimpleLineChart
          ...
          {analytics.topRatedGames && analytics.topRatedGames.length > 0 && (
            <View style={styles.section}>
